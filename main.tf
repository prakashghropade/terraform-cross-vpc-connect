...
resource "aws_vpc" "primary_vpc" {
  ...
  tags = {
    Name = "Priamry-VPC-${var.primary}"
  }
}

# second vpc
resource "aws_vpc" "secodary_vpc" {
  cidr_block       = var.secodary_vpc_cidr
  provider = aws.secondary
  enable_dns_hostnames = true
  enable_dns_support = true

  tags = {
    Name = "Secodary-VPC-${var.secondary}"
  }
}

# subnet for the primary subnet
...
# internet gatway of the primary vpc/subnet
...
# internet gatway  of the secondary vpc/subnet
resource "aws_internet_gateway" "secondary_igw" { 
    provider = aws.secondary
    vpc_id = aws_vpc.secodary_vpc.id

     tags = {
      Name = "Secondary-IGW-${var.primary}"
      Environment = "Demo"
    }
}
...
# route table association for primary vpc/subnet
...
# route table assiciation for the secondary vpc/subnet
...
# vpc peering from primary to secondary
resource "aws_vpc_peering_connection" "primary_to_secondary" {
    provider = aws.primary
    vpc_id = aws_vpc.primary_vpc.id
    peer_vpc_id = aws_vpc.secodary_vpc.id
...
# vpc peering from secondary to primary
...
# add the route in the priamry rt
resource "aws_route" "primary_to_secondary" {
    provider = aws.primary
    route_table_id = aws_route_table.primary_rt.id
    destination_cidr_block = var.secodary_vpc_cidr
    vpc_peering_connection_id = aws_vpc_peering_connection.primary_to_secondary.id

    depends_on = [ aws_vpc_peering_connection_accepter.secondary_accepter ]
}

# aws route from secondary to primary
...
# security group for the primary vpc EC2 instance
resource "aws_security_group" "primary_sg" {
   provider = aws.primary
   name = "primary-vpc-sg"
   description = "Security  group for the  primary VPC instance"
   vpc_id = aws_vpc.primary_vpc.id
   ...
   ingress {
    ...
    to_port = -1
    protocol = "icmp"
    cidr_blocks = [var.secodary_vpc_cidr]
   }
...
# security group for the secondary vpc instance
resource "aws_security_group" "secondary-sg" {
    provider = aws.secondary
    name = "secondary-vpc-sg"
    description = "Security group for the secondary vpc instance"
    vpc_id = aws_vpc.secodary_vpc.id
